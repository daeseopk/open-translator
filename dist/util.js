...
export function generateNodeId(node) {
    ...
    while (current && current.parentNode) {
        const siblings = Array.from(current.parentNode.childNodes).filter((sibling) => sibling.nodeName === current.nodeName);
...
export function collectAutoTranslateNodes(node, result = []) {
