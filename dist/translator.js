...
async function translateText(textArr, options) {
    const prompt = `
...
export async function translateTextBatch(texts, options) {
    const messages = await translateText(texts, options);
    if (texts.length !== messages.length) {
        console.warn(`원본 배열 길이(${texts.length})와 번역된 배열 길이(${messages.length})가 다릅니다.`);
    }
    // 한 번에 묶어서 출력
    const paired = texts
        .map((original, i) => {
